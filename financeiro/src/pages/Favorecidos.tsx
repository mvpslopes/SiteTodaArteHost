...
export default function Favorecidos() {
  ...
  const load = () => {
    ...
    api.favorecidos.list(!mostrarInativos)
      .then((r) => setFavorecidos(r.favorecidos))
  ...
  const inativar = async (id: number) => {
    if (!confirm('Inativar este favorecido? Ele não aparecerá na lista ao lançar transações.')) return;
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-semibold text-gray-900 flex items-center gap-2">
          <Users className="w-7 h-7 text-primary-500" strokeWidth={1.8} />
          Favorecidos
        </h1>
                ...
                {favorecidos.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="py-8 text-gray-500 text-center">
                      Nenhum favorecido. Cadastre pessoas ou instituições que recebem pagamentos.
                    </td>
                  </tr>
                ) : (
                  favorecidos.map((f) => (
