    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2 items-center">
          <button
            className={`px-4 py-2 rounded-xl shadow font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-logo/50 bg-gradient-to-r from-logo to-logo-light text-white ${
          ...
          <button
            className="flex items-center gap-2 bg-gradient-to-r from-logo to-logo-light text-white px-6 py-2 rounded-2xl shadow font-semibold transition-all duration-200 hover:brightness-110 focus:outline-none focus:ring-2 focus:ring-logo/50 text-lg"
      ...
      {showForm && (
        <form className="bg-white p-6 rounded-xl shadow mb-6 flex flex-wrap gap-4 items-end border border-logo-light/30" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button type="button" className="px-5 py-2 rounded-xl bg-gradient-to-r from-logo to-logo-light text-white hover:brightness-110 border-none" onClick={() => setShowForm(false)}>Cancelar</button>
          </div>
        </form>
      )}
      {view === 'tabela' && (
          ...
          <table className="min-w-fit text-base text-gray-800 w-full">
            ...
            <tbody>
              {atividadesPaginadas.map(a => (
                ...
                >
                  <td className="px-4 py-2 whitespace-normal break-words max-w-[220px] leading-tight" title={a.atividade}>
                    <div className="line-clamp-2 text-sm">{a.atividade}</div>
                  </td>
                  <td className="px-4 py-2 whitespace-normal break-words max-w-[160px] leading-tight" title={a.responsavel}>
                    <div className="line-clamp-1 text-sm">{a.responsavel}</div>
                  </td>
                  <td className="px-4 py-2 whitespace-normal break-words max-w-[160px] leading-tight" title={a.cliente}>
                    <div className="line-clamp-1 text-sm">{a.cliente}</div>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap text-center">
                    <div className="text-sm">{a.data_pedido ? formatDate(toZonedTime(parseISO(a.data_pedido), timeZone), 'dd/MM/yyyy') : '-'}</div>
                    <div className="text-xs text-gray-500">{a.data_pedido ? formatDate(toZonedTime(parseISO(a.data_pedido), timeZone), 'HH:mm') : ''}</div>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap text-center">
                    <div className="text-sm">{a.data_realizacao ? formatDate(toZonedTime(parseISO(a.data_realizacao), timeZone), 'dd/MM/yyyy') : '-'}</div>
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-center">
                    <div className="text-sm">{a.data_entrega ? formatDate(toZonedTime(parseISO(a.data_entrega), timeZone), 'dd/MM/yyyy') : '-'}</div>
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-center">
                    {a.arquivo ? (
                      <a href={a.arquivo} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center justify-center gap-1 text-sm" title="Abrir arquivo">
