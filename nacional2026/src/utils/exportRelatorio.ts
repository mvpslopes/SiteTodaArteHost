...
export async function exportDashboardExcel(data: DashboardData) {
  const resumo = [
    ...
    ['Saldo realizado', data.saldo],
    ['Saldo previsto (receber − pagar)', data.saldo_previsto],
    ['Total Clientes', data.total_clientes],
    ['Espaços Vendidos', `${data.espacos_vendidos} / ${data.total_espacos}`],
    [],
    ['A RECEBER'],
    ['Em aberto', data.a_receber_resumo.em_aberto],
    ['A vencer', data.a_receber_resumo.a_vencer],
    ['Em atraso', data.a_receber_resumo.atrasado],
    ['Recebido no ano', data.a_receber_resumo.recebido_ano],
    ['Qtd em aberto', data.a_receber_resumo.qtd_em_aberto],
    [],
    ['CONTAS A PAGAR'],
    ['Em aberto', data.a_pagar_resumo.em_aberto],
    ['A vencer', data.a_pagar_resumo.a_vencer],
    ['Em atraso', data.a_pagar_resumo.atrasado],
    ['Pago no ano', data.a_pagar_resumo.pago_ano],
    ['Qtd em aberto', data.a_pagar_resumo.qtd_em_aberto],
    [],
...
export async function exportDashboardPdf(data: DashboardData) {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Saldo realizado', formatMoney(data.saldo)],
      ['Saldo previsto', formatMoney(data.saldo_previsto)],
      ['Clientes', String(data.total_clientes)],
      ['Espaços vendidos', `${data.espacos_vendidos} / ${data.total_espacos}`],
      ['A receber em aberto', formatMoney(data.a_receber_resumo.em_aberto)],
      ['A receber em atraso', formatMoney(data.a_receber_resumo.atrasado)],
      ['A pagar em aberto', formatMoney(data.a_pagar_resumo.em_aberto)],
      ['A pagar em atraso', formatMoney(data.a_pagar_resumo.atrasado)],
    ],
    theme: 'grid',
